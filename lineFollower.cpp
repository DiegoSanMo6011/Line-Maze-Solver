...
void LineFollower::followSegment2()
{
    ...
    while (true) {
        ...
        const int maxSpeed = 70; // Velocidad máxima del motor 100
        if (powerDifference > maxSpeed) powerDifference = maxSpeed;
        if (powerDifference < -maxSpeed) powerDifference = -maxSpeed;
        ...
        if((sensors[0] < 170) && (sensors[2] < 170) && (sensors[4] < 170))//todos
        {
            return;

        }
        
        else if((sensors[0] < 170) && (sensors[4] < 170))//Izq & Derecha
        {
          return;
        }
        else if ((sensors[0] < 170) && (sensors[2] < 8)){  //iz & enfrente
          
          return;
        }
        else if ((sensors[4] < 170) && (sensors[2] < 8)){ // derecha & enfrente
